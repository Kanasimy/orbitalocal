		...
		<table id="basket_items">
			...
			<tbody>
				...
				foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
					...
					?>
					<tr id="<?=$arItem["ID"]?>"
                                            data-item-id="<?=$arItem["PRODUCT_ID"]?>"
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								<td class="custom">
									...
									if ($arHeader["id"] == "PROPERTY_CML2_ARTICLE_VALUE"){
                                                                        $prop=CIBlockElement::GetByID($arItem["PRODUCT_ID"])->GetNextElement();
...
<script>
    window.onload = function() {
            setTimeout(func, 3000);
                ...
                function func() {
                   var table_by_id =  document.getElementById('basket_items');
                   var rows = table_by_id.rows;
                   var name='{ "id":[';
                   var code;
for (var i=1; i<rows.length; i++)
{
    var out = {};
    code=rows[i].cells[1].getElementsByTagName('div')[0].getElementsByTagName('a')[0].href
    out.path = ( code.length > 0 ? "/" : "" ) + code.join( "/" );
    alert (out.path);
                if(i==1){
            name=name+rows[i].id;
        } else {
...
if (rezalt.activ[i]=='N'){
        var text=rows[i+1].cells[2].innerHTML;
    text=text+'<br>??? ?????';
    rows[i+1].cells[2].innerHTML=text;
     }
  }
      } else {
        // ????? ??????? ????????? ?? ??????
  ...
  // ?????????? ??? ???????
  request.open('post', '/property.php');
