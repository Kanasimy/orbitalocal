...
<?elseif($arResult["ALLOW_ANONYMOUS"]=="N" || $_REQUEST["authorize"]=="YES" || $_REQUEST["register"]=="YES"):?>
	...
	<tfoot><tr><td colspan="2"><input class="btn btn-primary" type="submit" name="Save" value="<?echo GetMessage("adm_auth_butt")?>" /></td></tr></tfoot>
	</table>
	...
	<?if($arResult["ALLOW_REGISTER"]=="Y"):
		...
		<tfoot><tr><td colspan="2"><input  class="btn btn-primary" type="submit" name="Save" value="<?echo GetMessage("adm_reg_butt")?>" /></td></tr></tfoot>
		</table>
		<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
			<input type="hidden" name="RUB_ID[]" value="<?=$itemValue["ID"]?>">
		<?endforeach;?>
