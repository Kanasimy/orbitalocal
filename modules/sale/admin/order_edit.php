	...
	if(($isSavingOperation || $isRefreshDataAndSaveOperation))
	{
		...
		if($res)
		{
			...
			if ($isRefreshDataAndSaveOperation)
			{
				...
				\Bitrix\Sale\SaleProviderBase::disableRefreshQuantity();

				$res = $basket->refreshData(array('PRICE', 'COUPONS'));

				\Bitrix\Sale\SaleProviderBase::enableRefreshQuantity();
...
<?else:?>
    <script type="text/javascript">
        ...
        function func() {
            var request = [];
            var elems = document.getElementsByClassName('adm-s-order-table-ddi-table-img');
            var note = document.getElementsByClassName('base_price_title');
            var numb = elems.length;
            var arrpos = [];
            var arrid = []
            for (var i = numb / 2; i < numb; i++) {
                var elem = elems[i].innerHTML;
                var start = elem.indexOf(';ID=');
                var end = elem.indexOf('&amp', start);
                var id = elem.slice(start + 4, end);
                var pos = i - numb / 2;
                arrid[pos] = id;
            }
            var name = '{ "id":[';
            ...
            request.onreadystatechange = function () {
                ...
                if (request.readyState == 4) {
                    ...
                    if (request.status == 200) {
                        var rezalt = JSON.parse(request.responseText);
                        for (i = 0; i < rezalt.activ.length; i++) {
                            if (rezalt.activ[i] == 'N') {
                                var text = note[i].innerHTML;
                                text = text + '<br>под заказ';
            ...
            request.open('post', '/local/property.php');
